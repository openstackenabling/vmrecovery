...
class RecoverDriver(object):
    ...
    def monitor_host(self, context):
        ...
        for host in current_dead_hosts:
            ...
            for dead_instance in dead_instances:
                ...
                self.recover_rpcapi.recover_vm(context, scheduler_host, port_id, port_mac, dead_instance_uuid, dead_instance_name)
    ...
    def _recover_net(self, port_id, port_mac, vm_uuid):
        ...
        shell_commands = (
            ...
            'sudo ovs-vsctl -- --may-exist add-port br-int qvo%s -- set Interface qvo%s  external-ids:iface-id=%s  external-ids:iface-status=active  external-ids:attached-mac=%s  external-ids:vm-uuid=%s' % (sub_port_id, sub_port_id, port_id, port_mac, vm_uuid)
            )
